...
print("List the nodes, separated by a comma: ")
node = str(input(""))

nodes = node.split(',')

print("List the edges separate by space: ")
edge = str(input(""))

edgeContainers = edge.split(' ')

edges = []

for i in range(len(edgeContainers)):
    edges.append(tuple(edgeContainers[i].split(',')))
...
#initialize G as undirected graph, and add nodes and edges
G = nx.Graph()
G.add_nodes_from(nodes)
G.add_weighted_edges_from(edges)

#mst is minimum spanning tree of graph G, with algorithm set to boruvka
mst = nx.minimum_spanning_tree(G,algorithm="boruvka")

#for calculating the total edge weight, taken from the mst variable
totalEdgeWeight = 0
...
nx.draw_networkx_edges(mst,pos,edge_color="r",label='weight')
